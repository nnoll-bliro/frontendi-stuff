...
export const CompaniesPage = () => {
  ...
  return (
    <>
      ...
      <Typography component="p" variant="xSmallBody" sx={{ mb: tokens.space.md }}>
      ...
      {companies.length === 0 ? (
        ...
        <SurfaceList>
          <Box sx={{ display: { xs: "none", md: "grid" }, gridTemplateColumns: "minmax(240px, 0.85fr) 1.3fr", gap: 4, px: tokens.space.lg, py: tokens.space.md, borderBottom: `1px solid ${colors.dark[700]}` }}>
            <Typography variant="eyebrow">Company</Typography>
            <Typography variant="eyebrow">Relationship overview</Typography>
          </Box>
          {companies.map((company) => (
            <Card key={company.id} sx={{ p: tokens.space.lg, display: "grid", gridTemplateColumns: { xs: "1fr", md: "minmax(240px, 0.85fr) 1.3fr" }, alignItems: "center", gap: { xs: 2, md: 4 } }}>
